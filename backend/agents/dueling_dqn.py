...
from .dqn import ReplayBuffer, _device, _clone_state, greedy_eval_return


class DuelingQNetwork(nn.Module):
    """
    Dueling architecture: shared backbone → separate Value and Advantage heads.
    ...
    def value_and_advantage(self, x: torch.Tensor):
        feat = self.backbone(x)
...
class DuelingDQNAgent:
    def __init__(
        ...
        self.device = _device()
        self.online = DuelingQNetwork(obs_dim, n_actions).to(self.device)
        self.target = DuelingQNetwork(obs_dim, n_actions).to(self.device)
    ...
    def get_value_advantage_maps(self, env) -> Dict:
        self.online.eval()
        value_map, adv_map, q_map = [], [], []
        with torch.no_grad():
            for s in range(env.n_states):
                state = env.idx_to_state(s)
                obs = env.state_to_obs(state)
                t = torch.tensor(obs, dtype=torch.float32, device=self.device).unsqueeze(0)
                v, a = self.online.value_and_advantage(t)
                q = v + a - a.mean(dim=1, keepdim=True)
                value_map.append(round(float(v.item()), 4))
                adv_map.append([round(float(x), 4) for x in a.cpu().numpy()[0]])
                q_map.append([round(float(x), 4) for x in q.cpu().numpy()[0]])
        self.online.train()
        return {"value_map": value_map, "advantage_map": adv_map, "q_map": q_map}
    ...
    def run_demo(self, env) -> Dict:
        ...
        if hasattr(env, "n_states"):
            maps = self.get_value_advantage_maps(env)
            result.update(maps)
            result["policy"] = [int(np.argmax(q)) for q in maps["q_map"]]
        return result
